...
import { pgTable, text, timestamp, boolean, index, decimal, jsonb, integer } from "drizzle-orm/pg-core";
import type { Seat, Reservation, Rating, Genre } from "../types";
...
export const auditorium = pgTable("auditorium", {
    ...
    seats: jsonb("seats").$type<Seat[]>().notNull().default([]),
});
...
export const screening = pgTable("screening", {
    ...
    auditoriumId: text("auditorium_id")
        .notNull()
        .references(() => auditorium.id, { onDelete: "cascade" }),
    startTime: timestamp("start_time").notNull(),
    endTime: timestamp("end_time").notNull(),
    price: decimal("price", { precision: 10, scale: 2 }).notNull(),
    takenSeats: jsonb("taken_seats").$type<Seat[]>().notNull().default([]),
});
...
export const user = pgTable("user", {
    ...
    reservations: jsonb("reservations").$type<Reservation[]>().default([]),
});
